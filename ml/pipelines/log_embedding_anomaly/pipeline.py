"""TF-IDF + IsolationForest pipeline over raw log lines (LogBERT-lite).

Uses TF-IDF char-ngrams as the embedding (CPU-cheap, no GPU). Swap to
DistilBERT-mini in `train.py` if GPU instances are approved.
"""
...
def build_pipeline(config):
    ...
    input_data_uri = ParameterString(
        name="input_data_uri",
        default_value=f"s3://{config.bucket}/processed/log-lines/",
    )
    ...
    validation_step = build_processing_step(
        ...
        arguments=["--min-rows", "1000"],
    )

    extract_step = build_processing_step(
        config=config, step_name="LogLineExtract",
    ...
    estimator = SKLearn(
        ...
        role=config.role_arn, instance_type=config.instance_type_train,
    ...
    register_step = build_register_step_with_gate(
        ...
        gate_op=">=", gate_threshold=config.metric_gate.get("precision_at_top1pct", 0.75),
        inference_instances=["ml.c5.xlarge", "ml.m5.large"],
    )
