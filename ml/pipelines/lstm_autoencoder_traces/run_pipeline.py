...
from ml.pipelines.lstm_autoencoder_traces.pipeline import build_pipeline
...
def main():
    ...
    config = PipelineConfig(
        model_name="lstm-ae-traces",
        model_group_name="LSTMAETracesModelGroup",
        environment=args.env, region=args.region,
        role_arn=args.role_arn, bucket=args.bucket,
        feature_group_name="monitoring-mlops-trace-sequences-v1",
        metric_gate={"auc": 0.80},
        instance_type_train="ml.g4dn.xlarge",
        instance_type_process="ml.m5.xlarge",
    )
