"""LSTM autoencoder over OTEL trace span sequences."""
...
def build_pipeline(config):
    ...
    from ml.pipelines._shared.pipeline_helpers import (
        build_evaluation_step, build_processing_step,
        build_register_step_with_gate, common_parameters, pipeline_session,
    )
    ...
    input_data_uri = ParameterString(
        name="input_data_uri",
        default_value=f"s3://{config.bucket}/processed/traces/",
    )

    validation_step = build_processing_step(
        config=config, step_name="DataValidation",
    ...
    extract_step = build_processing_step(
        config=config, step_name="TraceSequenceExtract",
    ...
    estimator = PyTorch(
        entry_point="train.py", source_dir=str(CODE_DIR),
        role=config.role_arn, instance_type="ml.g4dn.xlarge",
        ...
        hyperparameters={"epochs": 6, "batch_size": 32, "hidden_dim": 64, "lr": 1e-3, "max_seq_len": 64},
    )

    train_step = TrainingStep(
        name="Train", estimator=estimator,
        inputs={
            "train":    TrainingInput(s3_data=extract_step.properties.ProcessingOutputConfig.Outputs["train"].S3Output.S3Uri, content_type="application/json"),
            "metadata": TrainingInput(s3_data=extract_step.properties.ProcessingOutputConfig.Outputs["metadata"].S3Output.S3Uri, content_type="application/json"),
        },
    )

    eval_step, eval_pf = build_evaluation_step(
        config=config, code_path=str(CODE_DIR / "evaluate.py"),
    ...
    register_step = build_register_step_with_gate(
        config=config, training_step=train_step, eval_step=eval_step,
        eval_property_file=eval_pf,
        primary_metric_name="metrics.auc",
        gate_op=">=", gate_threshold=config.metric_gate.get("auc", 0.80),
